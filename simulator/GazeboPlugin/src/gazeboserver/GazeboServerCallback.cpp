    ...
    if (!ec && len >= EndOfMessage::getStr().size()) {
        std::string ss{
            ...
            boost::asio::buffers_end(buffer.data()) -
                static_cast<std::ptrdiff_t>(EndOfMessage::getStr().size())}; // Separate delimiter from buffer
        Packet request;
        ...
        else
        {
            ...
            replyMsg.set_status(Packet_CoSimError_MSG_INVALID);
        ...
        replyMsg.mutable_header()->mutable_time_stamp()->set_seconds(0);
        replyMsg.mutable_header()->mutable_time_stamp()->set_nano_seconds(0);
        replyMsg.set_status(Packet_CoSimError_SOCKET_FAILED);
        replyString = replyMsg.SerializeAsString();
    }
