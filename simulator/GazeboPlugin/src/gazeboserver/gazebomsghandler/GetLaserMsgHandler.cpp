...
std::string GetLaserMsgHandler::handleMessage(Packet const& msgContent) {
    Packet m_message;

    {
        ...
        if (sensor) {
            ...
            m_message.mutable_header()->set_id(PacketHeader_MsgID::PacketHeader_MsgID_STATUS);
            ...
            m_message.set_status(Packet_CoSimError_TOPIC_NAME_INVALID);
        }
    }
...
uint32_t GetLaserMsgHandler::getAcceptID() const {
    return PacketHeader_MsgID::PacketHeader_MsgID_REQUEST_LASER;
}
} // namespace gazebotransport
} // namespace robotics