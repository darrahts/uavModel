...
void SubscribeLaserMsgHandler::doSubscribe(gazebo::physics::WorldPtr world,
                                           gazebo::transport::NodePtr node,
...
std::string SubscribeLaserMsgHandler::getTopicName(Packet const& msgContent) {
    return msgContent.subscribe_laser().topic_name();
}

uint32_t SubscribeLaserMsgHandler::getAcceptID() const {
    return PacketHeader_MsgID::PacketHeader_MsgID_SUBSCRIBE_LASER;
}
} // namespace gazebotransport
} // namespace robotics
