...
std::string GetImuMsgHandler::handleMessage(Packet const& msgContent) {
    Packet new_message;

    {
        ...
        if (sensor) {
            ...
            new_message.mutable_header()->set_id(PacketHeader_MsgID::PacketHeader_MsgID_STATUS);
            ...
            new_message.set_status(Packet_CoSimError_TOPIC_NAME_INVALID);
        }
    }
...
uint32_t GetImuMsgHandler::getAcceptID() const {
    return PacketHeader_MsgID::PacketHeader_MsgID_REQUEST_IMU;
}
} // namespace gazebotransport
} // namespace robotics
