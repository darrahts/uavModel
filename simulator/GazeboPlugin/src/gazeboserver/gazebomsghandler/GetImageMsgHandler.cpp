...
std::string GetImageMsgHandler::handleMessage(Packet const& msgContent) {
    ...
    if (sensor) {
        ...
        message.mutable_header()->set_id(PacketHeader_MsgID::PacketHeader_MsgID_STATUS);
        message.mutable_header()->mutable_time_stamp()->set_seconds(static_cast<uint64_t>(simTime.sec));
        message.mutable_header()->mutable_time_stamp()->set_nano_seconds(static_cast<uint64_t>(simTime.nsec));
        message.set_status(Packet_CoSimError_TOPIC_NAME_INVALID);
...
uint32_t GetImageMsgHandler::getAcceptID() const {
    return PacketHeader_MsgID::PacketHeader_MsgID_REQUEST_IMAGE;
}
} // namespace gazebotransport
} // namespace robotics
