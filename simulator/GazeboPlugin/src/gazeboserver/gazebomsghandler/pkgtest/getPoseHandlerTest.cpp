...
class getPoseTest : public testing::Test
{
  ...
  public:
    ...
    /**
     * It creates and sends get ground truth world pose message and receives the reply message from server.
     * Further, it creates and returns Packet message from reply message.
     */
    robotics::gazebotransport::Packet clientGetPose(std::string const &modelName, std::string const &linkName)
    {
        /// Create Packet message to get pose of a link
        robotics::gazebotransport::Packet m_message;
        m_message.mutable_header()->set_id(robotics::gazebotransport::PacketHeader_MsgID::PacketHeader_MsgID_GET_GROUND_TRUTH_WORLD_POSE);
    ...
    void SetUp()
    {
        ...
        /// Launch Client
        m_client = std::make_shared<robotics::gazebotransport::Client>(ipAddress, m_server->getPortName(),boost::posix_time::milliseconds(time_out));
        ...
        std::thread imageTh0(&robotics::gazebotransport::GazeboServer::registerHandler, m_server,
