...
class maxStepSizeTest : public testing::Test
{
  public:
    ...
    void SetUp()
    {
        ...
        std::thread Th0(&robotics::gazebotransport::GazeboServer::registerHandler, m_server,
...
TEST_F(maxStepSizeTest, testGetStepSize)
{
    ...
    m_message.mutable_header()->set_id(robotics::gazebotransport::PacketHeader_MsgID::PacketHeader_MsgID_MAX_STEP_SIZE);
    m_message.mutable_header()->mutable_time_stamp()->set_seconds(0);
    m_message.mutable_header()->mutable_time_stamp()->set_nano_seconds(0);
    m_message.mutable_max_step_size()->set_type(robotics::gazebotransport::MaxStepSize_TYPE::MaxStepSize_TYPE_GET_STEP_SIZE);
...
TEST_F(maxStepSizeTest, testSetStepSize)
{
    ...
    m_message.mutable_header()->mutable_time_stamp()->set_seconds(0);
    m_message.mutable_header()->mutable_time_stamp()->set_nano_seconds(0);
    m_message.mutable_max_step_size()->set_type(robotics::gazebotransport::MaxStepSize_TYPE::MaxStepSize_TYPE_SET_STEP_SIZE);
    ...
    ASSERT_EQ(reply.status(), robotics::gazebotransport::Packet_CoSimError::Packet_CoSimError_NONE); // 0: NO Error
}
