...
class applyLinkWrenchTest : public testing::Test
{
  public:
     ...
     */
    robotics::gazebotransport::Packet clientApplyLinkWrench(std::string const &modelName, std::string const &linkName,
                            std::string const &forceType, std::string const &torqueType, double (&forceVal)[3], double (&torqueVal)[3], 
                            uint64_t sec, uint64_t nsec)
    {
        /// Create Packet message to apply force/torque on a link
        robotics::gazebotransport::Packet m_message;
        m_message.mutable_header()->set_id(robotics::gazebotransport::PacketHeader_MsgID::PacketHeader_MsgID_APPLY_LINK_WRENCH);
...
TEST_F(applyLinkWrenchTest, testForceTorqueAddSuccess)
{
    ...
    robotics::gazebotransport::Packet reply = clientApplyLinkWrench(modelName, linkName, "ADD", "ADD", 
                                                                    force_values ,torque_values, 0, 1000000);
    ASSERT_EQ(reply.status(),
              robotics::gazebotransport::Packet_CoSimError::Packet_CoSimError_NONE);
...
TEST_F(applyLinkWrenchTest, testForceTorqueSetSuccess)
{
    ...
    // apply force and torque for 0.001 second and check for the next iteration
    robotics::gazebotransport::Packet reply = clientApplyLinkWrench(modelName, linkName, "SET", "SET", 
                                                                    force_values ,torque_values, 0, 1000000);
    ASSERT_EQ(reply.status(),
              robotics::gazebotransport::Packet_CoSimError::Packet_CoSimError_NONE);
...
TEST_F(applyLinkWrenchTest, testForceTorqueAddTwiceSuccess)
{
    ...
    ASSERT_EQ(reply.status(),
              robotics::gazebotransport::Packet_CoSimError::Packet_CoSimError_NONE);
    reply = clientApplyLinkWrench(modelName, linkName, "ADD", "ADD", 
                                                                    force_values ,torque_values, 0, 1000000);
    ASSERT_EQ(reply.status(),
              robotics::gazebotransport::Packet_CoSimError::Packet_CoSimError_NONE);
...
TEST_F(applyLinkWrenchTest, testForceTorqueSetTwiceSuccess)
{
    ...
    force_values[0] = 1;
    reply = clientApplyLinkWrench(modelName, linkName, "SET", "SET", force_values ,torque_values, 0, 1000000);
    ASSERT_EQ(reply.status(),
              robotics::gazebotransport::Packet_CoSimError::Packet_CoSimError_NONE);
...
TEST_F(applyLinkWrenchTest, testForceTorqueAddSetSuccess)
{
    ...
    ASSERT_EQ(reply.status(),
              robotics::gazebotransport::Packet_CoSimError::Packet_CoSimError_NONE);
    reply = clientApplyLinkWrench(modelName, linkName, "SET", "SET", 
                                                                    force_values ,torque_values, 0, 1000000);
    ASSERT_EQ(reply.status(),
              robotics::gazebotransport::Packet_CoSimError::Packet_CoSimError_NONE);
...
TEST_F(applyLinkWrenchTest, testInValidModelName)
{
    ...
    robotics::gazebotransport::Packet reply = clientApplyLinkWrench("unt_box", "link", "ADD", "ADD", 
                                                                    force_values ,torque_values, 0, 0);

    ASSERT_EQ(reply.status(),robotics::gazebotransport::Packet_CoSimError::Packet_CoSimError_MODEL_NAME_INVALID); // MODEL_NAME_INVALID error
}
...
TEST_F(applyLinkWrenchTest, testInValidLinkName)
{
    ...
    robotics::gazebotransport::Packet reply = clientApplyLinkWrench("unit_box", "lik", "ADD", "ADD", 
                                                                    force_values ,torque_values, 0, 0);

    ASSERT_EQ(reply.status(),robotics::gazebotransport::Packet_CoSimError::Packet_CoSimError_LINK_NAME_INVALID); // LINK_NAME_INVALID error
}
...
TEST_F(applyLinkWrenchTest, testInValidForceType)
{
    ...
    robotics::gazebotransport::Packet reply = clientApplyLinkWrench("unit_box", "link", "AD", "ADD", 
                                                                    force_values ,torque_values, 0, 0);

    ASSERT_EQ(reply.status(),robotics::gazebotransport::Packet_CoSimError::Packet_CoSimError_FORCE_TYPE_INVALID); // FORCE_TYPE_INVALID error
}
...
TEST_F(applyLinkWrenchTest, testInValidTorqueType)
{
    ...
    robotics::gazebotransport::Packet reply = clientApplyLinkWrench("unit_box", "link", "ADD", "AD", 
                                                                    force_values ,torque_values, 0, 0);

    ASSERT_EQ(reply.status(),robotics::gazebotransport::Packet_CoSimError::Packet_CoSimError_TORQUE_TYPE_INVALID); // TORQUE_TYPE_INVALID error
}
