...
public:
     ...
     */
    gazebo::msgs::ImageStamped getImageMessage(int &width, int &height, int &ImageDepth, std::string const &dataString)
    {
    ...
    void smallImagePublisher()
    {
        ...
        gazebo::transport::PublisherPtr pub = node->Advertise<gazebo::msgs::ImageStamped>("/gazebo/default/camera/link/camera/image");
        ...
        while (this->clientEnd)
        {
            ...
            gazebo::msgs::ImageStamped imgMsg = getImageMessage(imgSmallWidth, imgSmallHeight, ImageDepth, imageString);
            pub->Publish(imgMsg);
    ...
    void largeImagePublisher()
    {
        ...
        /// Start gazebo publisher
        gazebo::transport::PublisherPtr pub = node->Advertise<gazebo::msgs::ImageStamped>("/gazebo/default/camera1/link/camera/image");
        ...
        {
            // Get & publish ImageStamped data
            gazebo::msgs::ImageStamped imgMsg = getImageMessage(imgLargeWidth, imgLargeHeight, ImageDepth, imageString);
            pub->Publish(imgMsg);
    ...
    robotics::gazebotransport::Packet clientSubscribeImage(std::string const &topic_name)
    {
        ...
        m_message.mutable_header()->set_id(robotics::gazebotransport::PacketHeader_MsgID::PacketHeader_MsgID_SUBSCRIBE_IMAGE);
    ...
    robotics::gazebotransport::Packet clientGetImage(std::string const &topic_name)
    {
        ...
        robotics::gazebotransport::Packet m_message;
        m_message.mutable_header()->set_id(robotics::gazebotransport::PacketHeader_MsgID::PacketHeader_MsgID_REQUEST_IMAGE);
        m_message.mutable_header()->mutable_time_stamp()->set_seconds(0);
        m_message.mutable_header()->mutable_time_stamp()->set_nano_seconds(0);
    ...
    void SetUp()
    {
        ...
        m_server = std::make_shared<robotics::gazebotransport::GazeboServer>(static_cast<uint16_t>(std::stoi("0")));
        m_server->run();
        /// Launch Client
        m_client = std::make_shared<robotics::gazebotransport::Client>(ipAddress, m_server->getPortName(),
        ...
        m_server->registerHandler(std::make_shared<robotics::gazebotransport::SubscribeImageMsgHandler>(world, nodeSub));
        m_server->registerHandler(std::make_shared<robotics::gazebotransport::GetImageMsgHandler>(world));
...
TEST_F(imagePublish, testSmallImageSensor)
{
    ...
    robotics::gazebotransport::Packet reply0 = clientSubscribeImage("/gazebo/default/camera/link/camera/image");
    ...
    robotics::gazebotransport::Packet replyGetImage0 = clientGetImage("/gazebo/default/camera/link/camera/image");
...
TEST_F(imagePublish, testInValidTopic)
{
    ...
    robotics::gazebotransport::Packet reply0 = clientSubscribeImage("/gazebo/default/camera0/link/camera/image");
    ...
    ASSERT_EQ(reply0.status(),robotics::gazebotransport::Packet_CoSimError::Packet_CoSimError_TOPIC_NAME_INVALID); 
...
TEST_F(imagePublish, testGetInvalidImage)
{
    ...
    robotics::gazebotransport::Packet replyGetWrongImage = clientGetImage("/gazebo/default/camera0/link/camera/image");
    ...
    // it should be false as topic is not available in gazebo
    ASSERT_FALSE(m_success0);
    ASSERT_EQ(replyGetWrongImage.status(),robotics::gazebotransport::Packet_CoSimError::Packet_CoSimError_TOPIC_NAME_INVALID); 
...
TEST_F(imagePublish, testLargeImage)
{
    ...
    robotics::gazebotransport::Packet reply0 = clientSubscribeImage("/gazebo/default/camera1/link/camera/image");
    ...
    robotics::gazebotransport::Packet replyGetLargeImg = clientGetImage("/gazebo/default/camera1/link/camera/image");
