...
public:
    ...
    void SetUp()
    {
        ...
        /// Launch Client
        m_client = std::make_shared<robotics::gazebotransport::Client>(ipAddress, m_server->getPortName(),boost::posix_time::milliseconds(10000));
        
        /// Start Gazebo Simulator Client
        gazebo::client::setup();
        
        /// Launch GetTopicList message handler
        m_server->registerHandler(std::make_shared<robotics::gazebotransport::GetTopicListMsgHandler>());
...
TEST_F(getTopicList, testTopicList)
{
    ...
    m_message.mutable_header()->set_id(robotics::gazebotransport::PacketHeader_MsgID::PacketHeader_MsgID_GET_TOPIC_LIST);
    ...
    //ASSERT_STREQ("/gazebo/default/atmosphere", reply.topic_list().data(0).name().c_str());      ///Gecko #g1990794
    ASSERT_STREQ("/gazebo/default/physics/contacts", reply.topic_list().data(0).name().c_str());
    //ASSERT_STREQ("/gazebo/default/diagnostics", reply.topic_list().data(2).name().c_str());    ///Gecko #g1990794
    //ASSERT_STREQ("/gazebo/default/factory", reply.topic_list().data(3).name().c_str());
    //ASSERT_STREQ("/gazebo/default/gui", reply.topic_list().data(4).name().c_str());         ///Gecko #g1990794
