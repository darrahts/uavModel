...
std::string ApplyLinkWrenchMsgHandler::handleMessage(Packet const& msgContent) {
            ...
            if (forceType == "SET" || forceType == "ADD") {
                if (torqueType == "SET" || torqueType == "ADD") {
                    ...
                    if (forceType == "SET") {
                        ...
                        // name )
                        m_commander.insertSetLinkCommand(msgContent.apply_link_wrench().model_name() + msgContent.apply_link_wrench().link_name(),
                                                         "FORCE", command);
                    ...
                    replyMsg.set_status(Packet_CoSimError::Packet_CoSimError_NONE); // NO ERROR
                } else {
                    replyMsg.set_status(
                        Packet_CoSimError::Packet_CoSimError_TORQUE_TYPE_INVALID); // TORQUE TYPE is
                                                                                   // incorrect
                }
            } else {
                replyMsg.set_status(
                    Packet_CoSimError::Packet_CoSimError_FORCE_TYPE_INVALID); // FORCE TYPE is
                                                                              // incorrect
            }
        } else {
            replyMsg.set_status(
                Packet_CoSimError::Packet_CoSimError_LINK_NAME_INVALID); // LINK not available
        }
    } else {
        replyMsg.set_status(
            Packet_CoSimError::Packet_CoSimError_MODEL_NAME_INVALID); // MODEL not available
    }
...
uint32_t ApplyLinkWrenchMsgHandler::getAcceptID() const {
    return PacketHeader_MsgID::PacketHeader_MsgID_APPLY_LINK_WRENCH; // Message ID from MsgTable
}
} // namespace gazebotransport
} // namespace robotics
