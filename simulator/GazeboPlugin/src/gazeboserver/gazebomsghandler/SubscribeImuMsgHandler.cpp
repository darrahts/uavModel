...
void SubscribeImuMsgHandler::doSubscribe(gazebo::physics::WorldPtr world,
                                         gazebo::transport::NodePtr node,
...
std::string SubscribeImuMsgHandler::getTopicName(Packet const& msgContent) {
    return msgContent.subscribe_imu().topic_name();
}

uint32_t SubscribeImuMsgHandler::getAcceptID() const {
    return PacketHeader_MsgID::PacketHeader_MsgID_SUBSCRIBE_IMU;
}
} // namespace gazebotransport
} // namespace robotics
