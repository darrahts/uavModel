...
std::string RequestCoSimulationHandler::handleMessage(Packet const& msgContent) {
    bool success = false;
    ...
    if (success) {
        ...
        replyMsg.set_status(Packet_CoSimError::Packet_CoSimError_COSIM_FAILED);
    }
...
uint32_t RequestCoSimulationHandler::getAcceptID() const {
    return PacketHeader_MsgID::PacketHeader_MsgID_REQUEST_COSIM;
}
} // namespace gazebotransport
} // namespace robotics
