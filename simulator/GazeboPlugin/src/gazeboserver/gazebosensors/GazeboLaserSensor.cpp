...
GazeboLaserSensor::GazeboLaserSensor(gazebo::physics::WorldPtr world, std::string const &topic)
    : GazeboSensorSubscriber(world, topic), m_subscriber(nullptr)
{
}
...
void GazeboLaserSensor::initImpl(gazebo::transport::NodePtr node)
{
    ...
    if (!m_subscriber)
    {
        m_subscriber = node->Subscribe(this->getTopicName(), &GazeboLaserSensor::updateMessage, this);
    }
}
...
void GazeboLaserSensor::updateMessage(ConstLaserScanStampedPtr &msg)
{
    ...
    m_lastMessage.mutable_header()->set_id(PacketHeader_MsgID::PacketHeader_MsgID_LASER_DATA);
    ...
    m_lastMessage.mutable_laser_data()->set_count(msg->scan().count());
    ...
    for (int i = 0; i < static_cast<int>(msg->scan().count()); i++)
    {
