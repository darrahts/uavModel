...
{
GazeboSensorSubscriber::GazeboSensorSubscriber(gazebo::physics::WorldPtr world, std::string const &topic)
    : m_world(world)
    , m_topicName(topic)
    , m_sensorName()
...
Packet GazeboSensorSubscriber::getLatestMessage()
{
...
void GazeboSensorSubscriber::resetMessageBuffer()
{
    std::lock_guard<std::mutex> lock(m_mutex);
    // initialize the message buffer
    m_lastMessage.mutable_header()->set_id(robotics::gazebotransport::PacketHeader_MsgID_STATUS);
    ...
    m_lastMessage.set_status(Packet_CoSimError_TOPIC_NAME_INVALID);
}
