...
function Update(block)
  ...
  % Xx = stateEqn(Xa(1:Nx,:),I,Xa(Nx+1:Nx+Nw,:)); 
    ...
    Ah = exp(-abs(n*current*G*dt/(3600*Q)));  % hysteresis factor
    xnew(2,:) = Ah.*xold(2,:) + (Ah-1).*sign(current);
    xnew(3,:) = xold(3,:) - n*current/3600/Q;
  ...
  %Y = outputEqn(Xx,u,Xa(nx+nw+1:end,:),battery);
    ...
    Y = getOCV_UKF(xhat(3,:), battery) + M*xhat(2,:) + M0*signIk - R*xhat(1,:) - R0*ik + ynoise(1,:);
  ...
  % Q-bump code
  if r^2>1*SigmaY, % bad voltage estimate by 2-SigmaX, bump Q 
    ...
    block.Dwork(1).Data(4) = 3*sqrt(sigmax(3,3));
    block.Dwork(1).Data(5) = sum(yhat)/length(yhat);
